...
class Player {
  constructor(x, y, color, spriteImg, leftKey, rightKey, upKey, downKey, shootKey) {
    ...
    // 精靈動畫設定
    this.currentFrame = 0;
    this.frameCount = 3;  // 每行有3幀
    this.frameWidth = 32;  // 原始圖片中每幀的寬度
    this.frameHeight = 32; // 原始圖片中每幀的高度
  ...
  display() {
    ...
    if (this.spriteImg) {
      ...
      let sourceY = 0;  // 如果有多行動畫，這裡可以調整

      // 繪製精靈
      image(this.spriteImg, 
            -this.size/2, -this.size/2,  // 調整為中心點定位
            this.size, this.size,        // 顯示大小
            sourceX, sourceY,            // 來源圖片的起始位置
            this.frameWidth, this.frameHeight  // 來源圖片的大小
      );
